...
class GameBody extends Component {
  ...
  makeMove(cellIndex, rowIndex) {
    if (cellIndex > 0 && this.state.gameData[rowIndex][cellIndex - 1] === 0) {
      ...
      rowIndex < this.state.gameData.length - 1 && this.state.gameData[rowIndex + 1][cellIndex] === 0
    ) {
      this.changeCoordinates(rowIndex + 1, cellIndex, rowIndex, cellIndex);
    } else if (
      cellIndex < this.state.gameData.length - 1 && this.state.gameData[rowIndex][cellIndex + 1] === 0
    ) {
      this.changeCoordinates(rowIndex, cellIndex + 1, rowIndex, cellIndex);
    } else if (
      rowIndex > 0 && this.state.gameData[rowIndex - 1][cellIndex] === 0
    ) {
      this.changeCoordinates(rowIndex - 1, cellIndex, rowIndex, cellIndex);
    }
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.isGameFinished() && this.state.gameStarted && !this.state.gameFinished
    ) {
      this.setState(_ => ({
        gameFinished: true
      }));
